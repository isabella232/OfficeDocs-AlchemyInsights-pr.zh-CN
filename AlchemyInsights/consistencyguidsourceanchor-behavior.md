 ...
 **注意：** 将本地 AD 对象导入到 Azure AD 连接 (即导入到 AD 连接器空间并投影到元节) 后，您将无法再更改它的 sourceAnchor 值。 若要指定给定内部部署 AD 对象的 sourceAnchor 值，请先配置其 ConsistencyGuid 属性，然后再将其导入到 Azure AD Connect。 
