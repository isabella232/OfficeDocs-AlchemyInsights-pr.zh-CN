...
或者，如果有对设备本身的管理访问权限，则可以通过从提升的命令提示符运行以下命令来查看恢复密钥 (密码) ：
...
```
如果在 Intune 中的 enrolment 之前加密了设备，则恢复密钥可能已与 "Microsoft 帐户" (MSA) 相关联，用于在 OOBE 过程中登录设备。 如果是这样，则  https://onedrive.live.com/recoverykey 使用该 MSA 访问和登录时应显示已存储恢复密钥的设备。
